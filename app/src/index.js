...
const App = {
  ...
  lookUp: async function (){
    ...
    const resName = document.getElementById("resultStarNameContent");
    const resSymbol = document.getElementById("resultStarSymbolContent");

    const {lookUptokenIdToStarInfo} = this.meta.methods;
    const {tokenIdToStarInfo} = this.meta.methods;
    await lookUptokenIdToStarInfo(lookid).send({from: this.account});
    var res = tokenIdToStarInfo(lookid).call().then(function(res){
      resName.innerHTML =  res["name"];
      resSymbol.innerHTML =  res["symbol"];
    });
  }
